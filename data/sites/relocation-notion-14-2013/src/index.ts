...
import { renderGoogleTagScript, renderPage, renderTocPage } from './template'
...
app.use('*', async (c, next) => {
  ...
  const legacyRedirect = redirectLegacyDomain(c.req.url, config.domain, config.legacyDomain)
...
app.get('/sitemap.xml', async (c) => {
  ...
  const staticEntries = staticEntriesWithToc(config)
...
app.get('/toc', async (c) => {
  ...
  let pages = await listCachedPageMetadata(c.env)
  if (!pages.length) {
    try {
      pages = await schedulePageIndexRefresh(c.env, config)
    } catch (error: any) {
      console.error('TOC refresh failed:', error?.message || error)
    }
  }

  return c.html(renderTocPage(config, pages))
})
...
function buildIndexedSitemapEntries(
  ...
  return sitemapEntriesFromIndexedPages(pages, staticEntriesWithToc(config))
}

function staticEntriesWithToc(config: ReturnType<typeof getConfig>): SitemapEntry[] {
  return config.enableToc
    ? [...staticSitemapEntries(config), { path: '/toc' }]
...
function redirectLegacyDomain(requestUrl: string, canonicalDomain: string, legacyDomain?: string): Response | null {
  if (!legacyDomain) {
    return null
  }

  const url = new URL(requestUrl)
  if (url.hostname !== legacyDomain) {
    return null
  }
